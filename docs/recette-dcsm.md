...
| 14  | Survoler puis cliquer l'icône ↻ à droite du bandeau de source                                                                            | Infobulle « Actualiser la liste des scripts ». L'icône tourne, le bandeau passe à « Liste à jour » avec l'heure courante, sans rechargement de la page ni perte des cases cochées non sauvegardées. |
| 15  | Mode développeur, activer **Agenda perso**, sauvegarder ; dans l'agenda, activer les rappels avec un événement dans la fenêtre de rappel | Une notification du navigateur s'affiche, sans `ReferenceError: GM_notification is not defined` : le DDK repasse bien `GM_notification` aux scripts chargés.                                        |
| 16  | Réinitialiser                                                                                                                            | Mémoire vidée, retour à l'état d'installation.                                                                                                                                                      |
...
scripts qui se chargent toujours. Cela couvre aussi l'étape 16.
