...
function addClickEvent(node, str) {
  if (navigator.userAgent.indexOf('Firefox') != -1) {
    node.children[0].addEventListener(
      'click',
      function () {
        unsafeWindow.$(this).next().toggle();
        unsafeWindow.$(this).find('.symbol').toggle();
        unsafeWindow.writeCookie(
          'forum_derniers_sujets_' + str,
          unsafeWindow.$(this).find('.symbol:first').css('display') == 'none'
            ? 'off'
            : 'on',
        );
      },
    ...
    node.children[0].setAttribute(
      'onclick',
      "$(this).next().toggle();$(this).find('.symbol').toggle();writeCookie('forum_derniers_sujets_" +
        str +
        "',$(this).find('.symbol:first').css('display')=='none'?'off':'on');",
    );
  }
}
...
(function () {
  ...
  sortSection(hrp, function (node, i, id) {
    if (id > 10 && id < 26) {
  ...
  sortSection(rp, function (node, i, id) {
    if (id <= 10 || id >= 26) {
      rp.children[1].removeChild(rp.children[1].children[i]);
      --i;
    }
    return i;
  });
