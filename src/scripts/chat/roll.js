...
const getStats = function () {
  ...
  for (let i = 1; i <= 8; i++) {
    stats.push(
      parseInt($('#statistiques .infos .stat_' + i + '_entier').text()),
    );
  }
...
const computeValue = function (skillName) {
  ...
  const totalWeight = formulas[skillName].reduce(
    (total, weight) => total + weight,
    0,
  );
  const skillPoints = stats.reduce(
    (total, stat, i) => total + stat * formulas[skillName][i],
    0,
  );
...
const retrieveValue = function (facesde, comp, diff) {
  ...
  if (
    ligne.includes(
      $('txt_pseudo').text() + ' lance 1 dé de ' + facesde + ' et fait',
    )
  ) {
    let result =
      parseInt(ligne.substring(ligne.indexOf('et fait') + 7).trim()) +
      (100 - facesde);
    let threshold = 0;
