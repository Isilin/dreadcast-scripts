...
$(document).ready(function () {
  ...
  const DEFAULT_CHAT_COLOR5 =
    $('.couleur5').css('color') !== undefined
      ? rgb2hex($('.couleur5').css('color'))
      : '#999';
  $('<div class="couleur4" style="display:none;"></div>').appendTo('body');
  const DEFAULT_CHAT_COLOR4 =
    $('.couleur4').css('color') !== undefined
      ? rgb2hex($('.couleur4').css('color'))
      : '#999';
  $('<div class="couleur_rouge" style="display:none;"></div>').appendTo('body');
  const DEFAULT_CHAT_COLOR_RED =
    $('.couleur_rouge').css('color') !== undefined
      ? rgb2hex($('.couleur_rouge').css('color'))
      : '#D32929';
  ...
  const DEFAULT_ALERT_CHAT_AUDIO_URL =
    'https://www.dreadcast.net/sons/dcce.mp3';
  ...
  var mrpCommandsString = initLocalMemory(
    JSON.stringify(mrpDefaultCommands),
    'DCCE_MRP',
  );
  var mrpCommands = JSON.parse(mrpCommandsString);

  var scriptZoneDroiteBG = initLocalMemory(
    DEFAULT_SCRIPT_ZONE_DROITE_BG,
    'DCCE_scriptZoneDroiteBG',
  );
  var scriptZoneChatBG = initLocalMemory(
    DEFAULT_SCRIPT_ZONE_CHAT_BG,
    'DCCE_scriptZoneChatBG',
  );

  var alertChatAudioURL = initLocalMemory(
    DEFAULT_ALERT_CHAT_AUDIO_URL,
    'DCCE_alertChatAudioURL',
  );
  ...
  var chatEmoteStyle = $(
    '<style id="chatEmoteStyle">span[style*="color:#58DCF9;"] em{color: ' +
      DEFAULT_CHAT_COLOR +
      ';}</style>',
  ).appendTo('head'); //Règle CSS pour appliquer la couleur du skin aux emotes. Visible uniquement côté client.
  var chatEmoteStyleWe = $(
    '<style id="chatEmoteStyleWe">.msg.couleur5 span[style*="color:#58DCF9;"] em{color: ' +
      DEFAULT_CHAT_COLOR5 +
      ';}</style>',
  ).appendTo('head');
  var chatEmoteStyleYe = $(
    '<style id="chatEmoteStyleYe">.msg.couleur_rouge span[style*="color:#58DCF9;"] em{color: ' +
      DEFAULT_CHAT_COLOR_RED +
      ';}</style>',
  ).appendTo('head');
  var chatEmoteStyleSpeech = $(
    '<style id="chatEmoteStyleSpeech">.msg em span[style*="color:#FFFFFF;"]{font-style: normal;}</style>',
  ).appendTo('head'); //Redresse les paroles dans des emotes
  ...
  var $dcce_background = $('<div id="dcce_background"></div>')
    .prependTo($('#zone_page'))
    .css({ left: '907px', top: '142px', height: '461px' })
    .css({ display: 'none' });

  function setZoneChatBackground() {
    if (chatExtend > 0) {
      $dcce_background.css({
        background: 'url("' + scriptZoneChatBG + '")',
        'background-size': '100% 100%',
        width: W_ZONE_CHAT + 13 + chatExtend + 'px',
      });
      $('#zone_droite').css({
        background: 'url("' + scriptZoneDroiteBG + '")',
      });
    } else {
      $dcce_background.css({
        background: 'none',
  ...
  $newMessageAlert.attr(
    'onmouseover',
    'this.style.backgroundColor="#0b9bcb";this.style.color="#FFFFFF";',
  );
  $newMessageAlert.attr(
    'onmouseout',
    'this.style.backgroundColor="#FFFFFF";this.style.color="#0296bb";',
  );

  //Initialisation bandeau latéral
  var $toggleAutoScroll = $(
    '<li id="toggleAutoScroll" class="couleur5" ></li>' +
...
.couleur5 .c${persoName}.couleur4 {color: ${DEFAULT_CHAT_COLOR4};}`;
  ...
  var DCCE_ConfigurationWindow = function () {
    ...
    var $config_window = $(
      '<div id="dcce_configwindow" onclick="engine.switchDataBox(this)"/>',
    );
    ...
    var $config_head = $(
      '<div class="head ui-draggable-handle" ondblclick="$(\'#dcce_configwindow\').toggleClass(\'reduced\');" />',
    ).appendTo($config_window);
    $(
      '<div title="Fermer la fenêtre (Q)" class="info1 link close" onclick="engine.closeDataBox($(this).parent().parent().attr(\'id\'));">X</div>',
    ).appendTo($config_head);
    $(
      '<div title="Reduire/Agrandir la fenêtre" class="info1 link reduce" onclick="$(\'#dcce_configwindow\').toggleClass(\'reduced\');">-</div>',
    ).appendTo($config_head);
    $('<div class="title">Configuration DC Enhanced Chat</div>').appendTo(
      $config_head,
    );
    ...
    var $autoconfig_title = $('<h2 class="couleur4 configTitre" />').appendTo(
      $autoconfig,
    );
    ...
    var $autoconfig_container = $('<div class="ligne"/>').appendTo(
      $config_interface,
    );
    ...
    var $autoconfig_radio_activate = $(
      '<input type="radio" name="typeAutoRadio" value="false">Activer</input>',
    ).appendTo($autoconfig_container);
    ...
    var $autoconfig_radio_deactivate = $(
      '<input type="radio" name="typeAutoRadio" value="true">Désactiver</input>',
    ).appendTo($autoconfig_container);
    ...
    //----------------------------------------
    var $scrconfig_container = $('<div class="ligne"/>').appendTo(
      $config_interface,
    );
    ...
    var $scrconfig_radio_activate = $(
      '<input type="radio" name="typeScrRadio" value="false">Afficher</input>',
    ).appendTo($scrconfig_container);
    ...
    var $scrconfig_radio_deactivate = $(
      '<input type="radio" name="typeScrRadio" value="true">Masquer</input>',
    ).appendTo($scrconfig_container);
    ...
    //Configuration de l'affichage de la barre de défilement
    //----------------------------------------
    var $predconfig_container = $('<div class="ligne"/>').appendTo(
      $config_interface,
    );
    ...
    var $predconfig_radio_activate = $(
      '<input type="radio" name="typePredRadio" value="false">Oui</input>',
    ).appendTo($predconfig_container);
    ...
    var $predconfig_radio_deactivate = $(
      '<input type="radio" name="typePredRadio" value="true">Non</input>',
    ).appendTo($predconfig_container);
    ...
    //----------------------------------------
    var $audioconfig_container = $('<div class="ligne"/>').appendTo(
      $config_interface,
    );
    ...
    var $audioconfig_pzonechat = $(
      "<div> </div><div>Audio joué à la réception d'un message (indiquer URL de l'audio) : </div>",
    ).appendTo($audioconfig_container);
    $audioconfig_pzonechat.css({
      margin: '0 5px',
      width: '500px',
    });
    var $audioconfig_zonechat = $(
      '<input type="url" name="typealertchat" value="' +
        alertChatAudioURL +
    ...
    var $audioconfig_vol = $(
      '<input type="range" name="typealertvolume" value="' +
        alertVolume +
    ...
    $audioconfig_vol.change(function () {
      ...
      document.getElementById('checkchat').volume = activateAlertChat
        ? alertVolume
        : 0;
    });
    ...
    var $clrconfig_title = $('<h2 class="couleur4 configTitre" />').appendTo(
      $clrconfig,
    );
    ...
    var $useritems_table = $('<table id="dcce_colorItems_config"/>').appendTo(
      $clrconfig,
    );
    $useritems_table.css({
      width: '100%',
    ...
    //Ligne d'en-têtes
    $useritems_table.append(
      $('<thead><tr><th>Personnage</th><th>Couleur</th><th></th></tr></thead>'),
    );
    ...
    for (let j = 0; j < localValues.length; j++) {
      if (localValues[j].includes('dcce_ctb_')) {
        ...
        var $perso_td = $(
          '<td class="perso_td" style="text-align:left;width:60%;font-size: 20px;padding-left: 5px;">' +
            item_perso +
            '</td>',
        ).appendTo($row);
        var item_couleur =
          '<input class="dcce_colortagbox" type="color" id="' +
          localValues[j] +
          '" value="' +
          GM_getValue(localValues[j]) +
          '"/>';
        var $couleur_td = $(
          '<td class="couleur_td" style="/*padding-left:10px;*/width:20%;text-align:center">' +
            item_couleur +
            '</td>',
        ).appendTo($row);
        ...
        $itemdel_btn.click(function () {
          if ($(this).data('confirmed')) {
            ...
            $(this)
              ...
              .children()
              .children('input')
              .val(DEFAULT_CHAT_COLOR); //Reset du color picker
    ...
    var $mrpconfig_title = $('<h2 class="couleur4 configTitre" />').appendTo(
      $clrconfig,
    );
    ...
    var $commands_table = $('<table id="dcce_commands_config"/>').appendTo(
      $clrconfig,
    );
    $commands_table.css({
      width: '100%',
    ...
    $commands_table.append(
      $(
        '<thead><tr><th>Alias</th><th>Nom</th><th>Couleur</th><th>Gras</th><th>Action</th></tr></thead>',
      ),
    );
    ...
    function updateCommand(id) {
      ...
      mrpCommands[id].bold =
        $('#mrp_bold_input_' + id).attr('checked') == 'checked' ? true : false;
      mrpCommands[id].rp =
        $('#mrp_rp_input_' + id).attr('checked') == 'checked' ? true : false;
    ...
    function drawCommand(cmd, id) {
      ...
      let $alias_td = $(
        '<td class="alias_td" style="text-align:left;font-size: 20px;padding-left: 5px;"><input style="background-color: antiquewhite" type="text" id="mrp_alias_input_' +
          id +
          '" value="' +
          cmd.alias +
          '"/></td>',
      ...
      let $name_td = $(
        '<td class="name_td" style="text-align:left;font-size: 20px;padding-left: 5px;"><input style="background-color: antiquewhite" type="text" id="mrp_name_input_' +
          id +
          '" value="' +
          cmd.name +
          '"/></td>',
      ...
      let $color_td = $(
        '<td class="color_td" style="text-align:center;font-size: 20px;"><input type="color" id="mrp_color_input_' +
          id +
          '" value="' +
          cmd.color +
          '"/></td>',
      ...
      let $bold_td = $(
        '<td class="bold_td" style="text-align:center;font-size: 20px;"><input type="checkbox" id="mrp_bold_input_' +
          id +
          '"' +
          (cmd.bold ? ' checked' : '') +
          '/></td>',
      ...
      let $rp_td = $(
        '<td class="rp_td" style="text-align:center;font-size: 20px;"><input type="checkbox" id="mrp_rp_input_' +
          id +
          '"' +
          (cmd.rp ? ' checked' : '') +
          '/></td>',
      ...
      $alias_td.children('input').keyup(function () {
        updateCommand(id);
      });
      $name_td.children('input').keyup(function () {
        updateCommand(id);
      });
      $color_td.children('input').change(function () {
        updateCommand(id);
      });
      $bold_td.children('input').change(function () {
        updateCommand(id);
      });
      $rp_td.children('input').change(function () {
        updateCommand(id);
      });
    }
    ...
    var $mrp_container = $('<div style="text-align: center"></div>').appendTo(
      $clrconfig,
    );
    var $mrp_remove = $(
      '<input style="color: red; background-color: antiquewhite; width: 45%; margin: 5px" type="button" id="mrp_remove" value="Supprimer une ligne" />',
    )
      .click(function () {
    ...
    var $mrp_add = $(
      '<input style="color: green; background-color: antiquewhite; width: 45%; margin: 5px" type="button" id="mrp_add" value="Ajouter une ligne" />',
    )
      .click(function () {
        drawCommand(
          { alias: '', name: '', color: '#FFFFFF', bold: false, rp: false },
          mrpCommands.length,
        );
        stylizeTableCells();
        mrpCommands.push({
  ...
  function chatCharCount(str) {
    ...
    return str
      .split('')
      .map((a) =>
        a.charCodeAt(0).toString(2).length >= 12
          ? 3
          : a.charCodeAt(0).toString(2).length >= 8
          ? 2
          : 1,
      )
      .reduce((a, b) => a + b);
  ...
  (function () {
    ...
    End.click(function () {
      ...
      document.getElementById('endAudiocheckchat').style.backgroundImage =
        activateAlertChat ? imgUnmute : imgMute;
      document.getElementById('checkchat').volume = activateAlertChat
        ? alertVolume
        : 0;
      var colorAC = activateAlertChat ? '#999' : '#D00000';
  ...
  function tagMessage(inputStr) {
    ...
    let command = mrpCommands.find(
      (cmd) => cmd.alias.toLowerCase() === aliasUsed.toLowerCase(),
    ); //On récupère les paramètres de la commande à partir de son alias (vérification non sensible à la casse)
    ...
    message =
      ...
      (command.bold ? '[b]' : '') +
      '[c=' +
      command.color.substr(1) +
      ']' +
      message.substr(command.alias.length + 1) +
      '[/c]' +
      (command.bold ? '[/b]' : '');
  ...
  MenuChat.prototype.send = function () {
    if (nav.getChat().kw('chatForm')) {
      if ('' == $('#chatForm input.text_chat').val()) return !1;
      this.checkConversation();
      if (
        $('#chatForm input.text_chat')
          .val()
          .split('')
          .map((a) =>
            a.charCodeAt(0).toString(2).length >= 12
          ...
          .reduce((a, b) => a + b) >= (0 == this.currentMode ? 199 : 195)
      )
        return !1; //La seule chose ajoutée par l'override: refuser d'envoyer le message si trop long! :)
      1 == this.currentMode
        ? $('#chatForm input.text_chat').val(
            '/we ' + $('#chatForm input.text_chat').val(),
          )
        : 2 == this.currentMode &&
          $('#chatForm input.text_chat').val(
            '/ye ' + $('#chatForm input.text_chat').val(),
          );
      engine.submitForm(
        'Menu/Chat/default=Send&etat=2&room=' + this.getRoom(!0),
        'chatForm',
        this.update,
        !0,
      ),
  ...
  function HighlightChatLimit() {
    ...
    var animateChatInput = function (e) {
      var processedInput = tagMessage(
        beautifyMessage($('#chatForm .text_chat').val()),
      );
      ...
      if (len >= maxLength) {
        ...
        $('#chatForm .text_chat').attr(
          'maxlength',
          $('#chatForm .text_chat').val().length,
        ); //On bloque pour que le joueur arrête d'écrire
  ...
  $('.text_chat').keydown(function (key) {
    //Si en préférence, scroller le chat automatiquement quand on commence à écrire.
  ...
  setInterval(function () {
    ...
    if (lastChat != $('#chatContent').text()) {
      ...
      if (autoScroll) {
        ...
        $('#chatContent .link.linkable:last').text() !== $('#txt_pseudo').text()
      ) {
  ...
  $(document).on('click', 'span.perso.link', function () {
    ...
    $(document).one(
      ...
      function (e, xhr, settings) {
        //Permet d'attendre le chargement de la fenêtre, one() pour éviter un duplicata.
        console.log(xhr);
        ...
        var colorTagBox = $(
          '<div style="margin-top: 15px; text-align: center;">' +
            ...
            idctb.slice(1) +
            '_reset" value="Reset" style="background-color: buttonface; margin-left: 5px; height: 16px; font-size: 12px;"/>' +
            '</div>',
        ).appendTo(
          $(
            '#ib_persoBox_' +
              e.data.idPerso +
              ' #char_inspect_faketooltip .content',
          ),
        );

        GM_setValue(
          'dcce_name_' + e.data.idPerso,
          $('#ib_persoBox_' + e.data.idPerso + ' .titreinfo')
            .contents()
            .filter(function () {
              return this.nodeType == 3;
            })[0].nodeValue,
        ); //Récupère le nom du personnage, utilisé dans le nommage de la classe du pseudo.

        if (GM_getValue('dcce_ctb_' + e.data.idPerso) !== undefined) {
          //Récupère et applique au bouton couleur la couleur enregistrée
          $(idctb).val(GM_getValue('dcce_ctb_' + e.data.idPerso));
        }
        $(idctb).on('change', { idPerso: e.data.idPerso }, function (e) {
          //Enregistre en mémoire la nouvelle couleur
          GM_setValue('dcce_ctb_' + e.data.idPerso, $(this).val());
          chatChangeColor(e.data.idPerso);
        });
        $(idctb + '_reset').on(
          'click',
          { idPerso: e.data.idPerso },
          function (e) {
            //Reset de la couleur
  ...
  MenuChat.prototype.update = function (a) {
    ...
    if ($(a).find('cydive').length) return cydiving.readChat(a), !0;
    $(a)
      .find('chat')
      .each(function () {
        if ($(this).attr('key'))
          ...
          if (
            a &&
            ($(
              '#zone_droite #zone_chat #chatContent, #chat_preview .chatContent',
            ).append(a),
            //$("#chatContent").scrollTop(jQuery("#chatContent").innerHeight()), //RETRAIT DE CETTE LIGNE
            Tutoriel.instance.tutoData.actif &&
              !Tutoriel.instance.isTutoCompleted(Tutoriel.EVENT_CHAT) &&
              $(this).find('span.link.linkable').length)
          ) {
            var b = $(this).find('span.link.linkable').attr('alt');
            engine.getIdPersonnage() != b &&
              $(window).trigger(Tutoriel.EVENT_CHAT);
          }
        }
      }),
      $(a).find('connectes').length &&
        $('#zone_chat .connectes').html($(a).find('connectes').xml());
    $('#zone_chat').height(),
      $('#zone_chat .connectes').height(),
      $('#zone_chat .chatForm').height(),
      $('#zone_chat').height(),
      $('#zone_chat .chatForm').height();
    $(a).find('chat').attr('time') &&
      nav.getChat().setTimeCurrentRoom($(a).find('chat').attr('time')),
