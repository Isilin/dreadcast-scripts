...
Engine.prototype.openPersoBox = function (a, b) {
  var c = this;
  return $('#zone_infoBoxFixed #ib_persoBox_' + a).length
    ? ($('#zone_infoBoxFixed #ib_persoBox_' + a).remove(), !0)
    : void $.post(
        './Main/FixedBox/PersoBox',
        {
          id: a,
        },
        function (d) {
          if ('ERROR1' != d) {
            ...
            $('#zone_infoBoxFixed #ib_persoBox_' + a + ' .case_objet').each(
              function () {
                e.updateEffectsCaseObjet($(this));
              },
            );
            $('#zone_infoBoxFixed #ib_persoBox_' + a)
              .hide()
              .fadeIn('fast')
              .draggable(),
              setOnTop('#zone_infoBoxFixed #ib_persoBox_' + a, 'infoBoxFixed'),
              $('#zone_infoBoxFixed #ib_persoBox_' + a).click(function () {
                $(this).hasClass('onTop') || setOnTop(this, 'infoBoxFixed');
              }),
              centrageBox(b, '#zone_infoBoxFixed #ib_persoBox_' + a, 30, 15),
              c.updateToolTip('.info1, .link_info1'),
              c.updateToolTip('.info2, .link_info2', 2);
            var f = parseFloat(
              $('#stat_6_entier').text() + $('#stat_6_decimal').text(),
            );
            0 == f &&
              $('#ib_persoBox_' + a + ' .interaction_3')
                .addClass('np')
                .attr('onClick', '');

            if (silhouettesUrl[a])
              $('#zone_infoBoxFixed #ib_persoBox_' + a + ' .personnage_image')
                .css('background-image', 'url(' + silhouettesUrl[a] + ')')
                .css('background-position', '0px 0px');
          }
        },
      );
