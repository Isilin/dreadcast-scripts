...
for (let i = 0; i < params.user_itemsNumber; i++) {
  ...
  if (GM_getValue(user_item) !== undefined) {
    //l'user_item existe en mémoire
    let user_item_icon = user_item + '_icon';
    let user_item_text = user_item + '_text';
    if (GM_getValue(user_item_icon) !== undefined) {
      //récupération de l'icône
      params.icons[user_item] = GM_getValue(user_item_icon);
    }
    if (GM_getValue(user_item_text) !== undefined) {
...
for (let i = 0; i < params.user_actionsNumber; i++) {
  ...
  if (GM_getValue(user_action) !== undefined) {
    //l'user_action existe en mémoire
    let user_action_icon = user_action + '_icon';
    let user_action_text = user_action + '_text';
    if (GM_getValue(user_action_icon) !== undefined) {
      //récupération de l'icône
      params.icons[user_action] = GM_getValue(user_action_icon);
    }
    if (GM_getValue(user_action_text) !== undefined) {
      //récupération du texte
...
let DCCB_ConfigurationWindow = function () {
  ...
  let $config_window = $(
    '<div id="dccb_configwindow" onclick="engine.switchDataBox(this)"/>',
  );
  ...
  let $config_head = $(
    '<div class="head ui-draggable-handle" ondblclick="$(\'#dccb_configwindow\').toggleClass(\'reduced\');" />',
  ...
  $(
    '<div title="Reduire/Agrandir la fenêtre" class="info1 link reduce" onclick="$(\'#dccb_configwindow\').toggleClass(\'reduced\');" >-</div>',
  ).appendTo($config_head);
  $('<div class="title">Configuration DC Com\'Back</div>').appendTo(
    $config_head,
  );
  ...
  $warning_banner.text(
    "ATTENTION : Veillez à ne pas modifier les paramètres de Com'Back si des messages sont actuellement ouverts.",
  );
  $warning_banner.css({
    color: '#f10a18',
    'text-align': 'center',
  ...
  let $bodyconfig_emote_color_picker = $('<input type="color" />').appendTo(
    $bodyconfig_emote_color,
  );
  ...
  //----------------------------------------
  let $listconfig_title = $('<h2 class="couleur4" />').appendTo(
    $config_interface,
  );
  ...
  let $listconfig_default = $('<div class="ligne"/>').appendTo(
    $config_interface,
  );
  ...
  let $listconfig_default_lastone = $(
    '<input type="radio" name="typeListDefault" value="false">Dernière utilisée</input>',
  ).appendTo($listconfig_default);
  ...
  let $listconfig_default_value = $(
    '<input type="radio" name="typeListDefault" value="true">Toujours :</input>',
  ).appendTo($listconfig_default);
  ...
  let $listconfig_default_picker =
    $('<select />').appendTo($listconfig_default);
  ...
  $listconfig_default_picker.css({
    ...
    'font-family':
      'Arial,Segoe UI Symbol,Unifont,Unifont Upper CSUR,sans-serif',
  });
  ...
  for (let i = 0; i < params.typeList.length; i++) {
    ...
    let $option = $('<option id="opt_' + type_id + '" />').appendTo(
      $listconfig_default_picker,
    );
  ...
  $listconfig_default_lastone.change(function () {
    if ($(this).attr('checked')) {
      //Utilisateur de la dernière valeur utilisée
      params.b_alwaysDefault = false;
      params.list_defaultID = 'NONE';
      $listconfig_default_picker.attr('disabled', 'disabled');
      GM_setValue('list_defaultID', params.list_defaultID);
  ...
  let $useritems_table = $('<table id="dccb_userItems_config"/>').appendTo(
    $listconfig_items,
  );
  $useritems_table.css({
    width: '100%',
  ...
  $useritems_table.append(
    $('<thead><tr><th>Symbole</th><th>Description</th><th></th></tr></thead>'),
  );
  ...
  for (let i = 0; i < params.user_typeList.length; i++) {
    ...
    let $icon_td = $(
      '<td class="editable" style="text-align:center;width:10%;font-size: 20px;">' +
        item_icon +
        '</td>',
    ).appendTo($row);
    ...
    let $text_td = $(
      '<td class="editable" style="padding-left:10px;width:70%;">' +
        item_text +
        '</td>',
    ).appendTo($row);
    ...
    $itemdel_btn.click(function () {
      if ($(this).data('confirmed')) {
        ...
        if ($listconfig_default_picker.val() === type_id) {
          //l'item à supprimer est sélectionné
  ...
  $('td.editable', $useritems_table).dblclick(function () {
    ...
    editCellContent($(this), function (changes) {
      if (changes) {
        ...
        let item_title =
          params.icons[type_id] + ' - ' + params.infotexts[type_id];
        if (params.typeList.indexOf(type_id) === -1) {
          ...
          //modification dans le menu déroulant si déjà présent
  ...
  //----------------------------------------
  let $buttonsconfig_title = $('<h2 class="couleur4" />').appendTo(
    $config_interface,
  );
  ...
  let $listconfig_actions = $('<div class="ligne"/>').appendTo(
    $config_interface,
  );
  $listconfig_actions.text('Boutons personnalisés : ');
  let $useractions_table = $('<table id="dccb_userActions_config"/>').appendTo(
    $listconfig_actions,
  );
  ...
  $reinit_btn.attr(
    'title',
    'Réinitialisation des variables et fermeture de la fenêtre',
  );
...
function carouselFacesNumber() {
  let integer_part = Math.floor(
    params.actionList.length / params.carousel_facesize,
  );
...
function makeBanner(CR_before, id, CR_after, isHeader) {
  ...
  return (
    Array(CR_before + 1).join('\n') +
    space.repeat(nbrSpace) +
    banner +
    Array(CR_after + 1).join('\n')
  );
}
...
function truncateString(string, nb_char) {
  return (
    $.trim(string).substring(0, nb_char).split(' ').slice(0, -1).join(' ') +
    '...'
  );
}
...
function editCellContent(cell, cb) {
  ...
  cell.html(
    '<input style="width:100%;background-color:rgb(200,200,210)" type="text" value="' +
      init_value +
      '" />',
  );
    ...
    .keypress(function (e) {
      if (e.which == 13) {
        ...
        if (new_value !== init_value) {
          //la nouvelle valeur est différente de l'ancienne
          return cb(new_value);
        } else {
...
function format_liens(html) {
  ...
  //Ta mère youtube
  html = html.replace(
    /<a\shref=\"(?:http?s?:\/\/)?(?:www\.)?(?:youtube\.com|youtu\.be)\/(?:watch\?v=)?(.+)\"\starget=\"_blank\">[\S]+<\/a>/gim,
    '<center><embed style="max-width: 355px;" src="https://www.youtube.com/embed/$1"></embed></center>',
  );
  //Transforme les liens de son en son...
  html = html.replace(
    /<a\shref=\"([\S]+(\.mp3|\.ogg|\.wav))\"\starget=\"_blank\">[\S]+<\/a>/gim,
    '<center><audio controls><source src="$1"></audio></center>',
  );

  //Transforme les liens d'images en images cliquables
  html = html.replace(
    /<a\shref=\"([\S]+(\.png|\.jpg|\.jpeg|\.gif))\"\starget=\"_blank\">[\S]+<\/a>/gim,
    '<center><a href="$1" target="_blank"><img src="$1" style="max-width: 355px;"></a></center>',
  );
...
function mainf() {
  ...
  var $msg_textarea = $(
    toContent + ' > ' + class_name + ' > #nm_texte > textarea',
  );
  ...
  $(db_id + ' > relative > .head > .info1.link.reduce')
    .attr('onclick', '')
    .click(function () {
  ...
  $types_selection.css({
    //TODO : vérifier que ellipsis fonctionne
    display: 'block',
    width: '165px',
    'white-space': 'nowrap',
    overflow: 'hidden',
  ...
  var type_id = params.b_alwaysDefault
    ? params.list_defaultID
    : params.list_lastID;
  ...
  $(db_id + ' > relative > .content > .message_nouveau > .envoyer.link')
    .attr('onclick', '')
    .click(function () {
  ...
  for (
    var idx_carousel = 0;
    idx_carousel < params.carousel_facesnumber;
    idx_carousel++
  ) {
  ...
  for (var idx_btn = 0; idx_btn < params.actionList.length; idx_btn++) {
    ...
    var $button = $(
      '<button title="' +
        params.infotexts[action_id] +
        '" class="cb_annexes" id="DCCB_b' +
        idx_btn.toString() +
        '">' +
        params.icons[action_id] +
        '</button>',
...
function filcomf() {
  $('#liste_messages').ajaxComplete(function (e, xhr, opt) {
    //Naugriim, je t'aime. <3 (Attendre le chargement de la fenêtre avant d'envoyer la sauce)
    ...
    $(db_id + ' > relative > .content > .message > .grid1 > .contenu').css(
      'min-height',
      '25vh',
    ); // Test pour hauteur de com' responsive...

    //Transformation des liens en liens cliquables
    //*********************
    let $message_content = $(
      db_id + ' > relative > .content > .message > .grid1 > .contenu > .texte',
    );
    ...
    $(db_id + ' .link.conversation').click(function () {
      let $cheminTexte = $(
        db_id +
          ' > relative > .content > .message > .grid1 > .contenu > .texte',
      );
      $cheminTexte.ajaxComplete(function () {
        $(this).unbind('ajaxComplete');
        let this_clicked_id = $(db_id + ' .link.conversation.selected').attr(
          'id',
        );
    ...
    $(db_id + ' > relative > .content > div.message > .grid2 .btnTxt')
      .filter((i, el) => ['Répondre', 'Inviter'].includes($(el).text()))
      .click(function () {
        //Création et affichage lors du clic sur l'un des boutons en bas de la fenêtre

        if (document.getElementById('dccb_div_fc_' + message_id) === null) {
          //Ne recrée pas l'interface du script si elle existe déjà: REND IMPOSSIBLE L'OUVERTURE DE PLUSIEURS COMS SANS BUGS.

          let $msg_content = $(db_id + ' > relative > .content > .message');
          let $msg_textarea = $(
            db_id +
              ' > relative > .content > .message > .zone_reponse > .texte > #nm_texte > textarea',
          );
          ...
          //Création du conteneur
          let $types_div = $(
            '<div id="dccb_div_fc_' + message_id + '" />',
          ).appendTo($msg_content);
          ...
          //Création de la liste
          let $types_selection = $(
            '<select id="listeTypesfc_' + message_id + '" />',
          ).appendTo($types_div);
          ...
          //Application du choix par défaut
          let type_id = params.b_alwaysDefault
            ? params.list_defaultID
            : params.list_lastID;
          $types_selection.val(type_id);
          if (type_id !== 'NONE') {
          ...
          $(
            db_id +
              ' > relative > .content > .message > .zone_reponse > .btnTxt',
          )
            .attr('onclick', '')
            .click(function () {
              //Ajout d'un en-tête au message avant l'envoi
          ...
          //Boutons annexes pour ajouter des bouts de texte (pièce jointe, etc).
          let $actions_div = $(
            '<div id="dccb_annexesfc_' + message_id + '"/>',
          ).appendTo($msg_content);
          $actions_div.css({
            'z-index': '999999',
            position: 'absolute',
          ...
          for (let idx_btn = 0; idx_btn < params.actionList.length; idx_btn++) {
            let dedicatedCarousel_id =
              'carousel_' + carouselDedicatedFaceIdx(idx_btn);
            let action_id = params.actionList[idx_btn];
            let $button = $(
              '<button title="' +
                params.infotexts[action_id] +
                '" class="cb_annexesfc cb_annexesfc_' +
                message_id +
                '" id="DCCB_b' +
                idx_btn.toString() +
                '_' +
          ...
          $msg_textarea.css({
            'font-family':
              'Verdana,Courier,Segoe UI Symbol,Unifont,Unifont Upper CSUR,sans-serif',
...
if (!checkData()) {
  ...
  setInterval(function () {
    //Fix bien crade pour contacter l'auteur d'une annonce AITL.
