...
$(document).ready(function () {
  ...
  $('#bandeau ul.menus')
    .eq(0)
    .prepend(
      '<li id="affichageDateDC" class="couleur5" ></li>' +
  ...
  $('head').append(
    '<style>/*DC time updated stylesheet*/.custom_command.important_data {       font-weight: bold;}.custom_command.red_info {       color: red;}.custom_command.green_info {        color: green;}.custom_command.orange_info {     color: orange;}</style>',
  );

  // Display DC's server date
  function handle_DC_date() {
    if (undefined === server_date) return;
  ...
  var CommandLine_DC_time = function (command_line) {
    ...
    if (/\-/gi.test(command_line)) {
      // argument given
      this.argument = command_line.trim().split('-')[1];

      if (/ /gi.test(this.argument)) {
        // parameter given
    ...
    this.enabledArguments = [
      'h', // display all date's and secondes.
    ...
    this.execute = function (deck) {
      ...
      if (false === CommandLine_DC_time.check_character_skill()) {
        deckLines.push(
          Deck.getHTMLElementLineTab([
            'span',
            'custom_command',
            'Votre niveau en informatique est trop faible pour réussir cette commande',
          ]),
        );
        ...
        switch (this.argument) {
          ...
          case 'h': // display all date's and secondes.
            // put separators
            var deckLine =
              $('#affichageDateDC').html() + ' ' + $('#affichageDate').html();
            deckLine = deckLine.replace(/ /g, ' | ');

            deckLines.push(
              Deck.getHTMLElementLineTab(['span', 'custom_command', deckLine]),
            );
            break;
          case 'd': // display converted standard date to DC's date
            ...
            try {
              ...
              deckLines.push(
                Deck.getHTMLElementLineTab(['span', 'custom_command', date]),
              );
            } catch (e) {
              deckLines.push(
                Deck.getHTMLElementLineTab(
              ...
              deckLines.push(
                Deck.getHTMLElementLineTab(
                  [
                    'span',
                    'custom_command',
                    'Point Temporel Impérial invalide (',
                  ],
  ...
  CommandLine_DC_time.check_character_skill = function () {
    if (
      CommandLine_DC_time.cumputing_skill_need <=
      $('.stat_6_entier').first().html()
    )
      return true;
  ...
  CommandLine_DC_time.getManualLines = function () {
    ...
    manualLines[manualLines.length] = Deck.getHTMLElementLineTab([
      'span',
      'custom_command couleur_jaune important_data',
      'date: ',
    ]);
    ...
    manualLines[manualLines.length] = Deck.getHTMLElementLineTab([
      'span',
      'custom_command',
      'Affiche la date ou en convertit les différents formats',
    ]);
    ...
    manualLines[manualLines.length] = Deck.getHTMLElementLineTab(
      ...
      [
        'span',
        'custom_command',
        " : Affiche tous les formats de date, ainsi que l'",
      ],
      ['span', 'custom_command couleur_jaune', 'h'],
  ...
  Deck.getHTMLElementLineTab = function () {
    ...
    for (var i = 0; i < arguments.length; i++) {
      lineTab[lineTab.length] = Deck.getHtmlElementTab(
        arguments[i][0],
        arguments[i][1],
        arguments[i][2],
      );
  ...
  $(document).ajaxComplete(function (a, b, c) {
    ...
    if (first_sync) old_server_date = server_date;
    ...
    if (/Command/.test(c.url)) {
      ...
      var commandLine_text = $('#' + deckId + ' .ligne_ecrite_fixed')
        .last()
        .find('input')
        .val();
