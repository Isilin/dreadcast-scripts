...
$(document).ready(function () {
  ...
  function initialize() {
    ...
    if (false === show_overlay_options) {
      ...
      if (false === full_auto)
        ...
        $('#auto_refresh .overlay_options .toggle').css({
          borderColor: 'green',
        });
    }
    if (true === full_auto) {
      $('#auto_refresh .overlay_options .full_auto .check_box').css({
        boxShadow: 'inset 0px 0px 0px 1px rgba(0, 210, 0,1)',
  ...
  function refresh_reload_alert(forced) {
    if (
      true === forced ||
      true === $('#auto_refresh .alert .seconds').is(':Visible')
    ) {
      $('#auto_refresh .alert .seconds').html(
        Math.round((refresh_time_limit - (Date.now() - reload_asked)) / 1000),
      );
    }
  }
  ...
  function refresh_ping(request_send, request) {
    ...
    if (false === disconnected) first_request_send = request_send;
    else {
    ...
    var ping =
      request - request_send < 0 // During requesting new Check
        ? ping
    ...
    if (ping > check_delay * 2) {
      ...
      $('#auto_refresh .overlay .actual_time').css(
        { width: '0px' },
    ...
    } else $('#auto_refresh .overlay .refresh_delay .unit').html('ms');
  ...
  function refresh_delay(request, animation_start) {
    ...
    if (undefined === animation_start) animation_start = 0;
    else {
    ...
    $('#auto_refresh .overlay .actual_time').css(
      {
        width:
          animation_progression *
            $('#auto_refresh .overlay .refresh_delay').width() +
          'px',
      },
  ...
  function refresh_reload_auto() {
    ...
    $('#auto_refresh .overlay_options .reload_auto .seconds').html(
      Math.round((refresh_time_limit - (Date.now() - reload_asked)) / 1000),
    );
  }
  ...
  $('#auto_refresh .overlay .ping').on('click', function () {
    ...
    saveCookie(
      'global',
      getCookie('global').replace(/(delay_anim_on|delay_anim_off)/gi, '') +
        (true === animation_enabled ? 'delay_anim_on' : 'delay_anim_off'),
    );
  });
  ...
  $('#auto_refresh .overlay_options .full_auto').on('click', function () {
    ...
    saveCookie(
      'global',
      getCookie('global').replace(/(full_auto_on|full_auto_off)/gi, '') +
        (true === full_auto ? 'full_auto_on' : 'full_auto_off'),
    );
  });
  ...
  $('#auto_refresh .overlay .toggle').on('click', function () {
    ...
    if (true === show_overlay) {
      ...
      $('#auto_refresh .overlay .toggle').html('>');
      $('#auto_refresh .overlay .toggle').css({
        borderColor: $('#auto_refresh .overlay .refresh_delay').css(
          'borderColor',
        ),
      });
    }
    ...
    saveCookie(
      'global',
      getCookie('global').replace(/(overlay_on|overlay_off)/gi, '') +
        (true === show_overlay ? 'overlay_on' : 'overlay_off'),
    );
  });
  ...
  $('#auto_refresh .overlay_options .toggle').on('click', function () {
    ...
    saveCookie(
      'global',
      getCookie('global').replace(
        /(overlay_options_on|overlay_options_off)/gi,
        '',
      ) +
        (true === show_overlay_options
          ? 'overlay_options_on'
          : 'overlay_options_off'),
    );
  });

  $('#auto_refresh .overlay_options .abort_reload_auto').on(
    'click',
    function () {
      abort_reload();
      refresh_reload_auto();
  ...
  function start_global_clock() {
    ...
    global_clock_interval = setInterval(function () {
      ...
      lag_buster =
        5 == server_date.getHours() ? hard_lag_buster : casual_lag_buster;
