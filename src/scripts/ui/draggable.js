...
DC_draggable['zone_gauche']['checkbox'] =
  '<div style="height:0px;width:0px;z-index:999999;margin-right:-13px;"><input type="checkbox" name="zone_gauche" class="DC_draggable"></div>';

DC_draggable['zone_droite'] = new Array();
DC_draggable['zone_droite']['checkbox'] =
  '<div style="height:0px;width:0px;z-index:999999;margin-right:-13px;"><input type="checkbox" name="zone_droite" class="DC_draggable"></div>';

DC_draggable['zone_informations_lieu'] = new Array();
DC_draggable['zone_informations_lieu']['checkbox'] =
  '<div style="height:0px;width:0px;z-index:999999;top:-11px;left:-15px;"><input type="checkbox" name="zone_informations_lieu" class="DC_draggable"></div>';
DC_draggable['zone_informations_lieu']['css_initial_optionnal'] = {
  top: 'auto',
};

DC_draggable['zone_informations_combat'] = new Array();
DC_draggable['zone_informations_combat']['checkbox'] =
  '<div style="height:0px;width:0px;z-index:999999;top:-11px;left:-15px;"><input type="checkbox" name="zone_informations_combat" class="DC_draggable"></div>';
DC_draggable['zone_informations_combat']['css_initial_optionnal'] = {
  top: 'auto',
};

DC_draggable['db_combat'] = new Array();
DC_draggable['db_combat']['checkbox'] =
  '<div style="height:0px;width:0px;z-index:999999;top:-36px;left:-10px;"><input type="checkbox" name="db_combat" class="DC_draggable"></div>';
// /SETTABLE
...
$(document).ready(function () {
  ...
  if (undefined != getCookie('preset')) preset = parseInt(getCookie('preset'));
  ...
  function set_DC_draggable(make_draggable) {
    ...
    for (var i = 0; i < to_drag.length; i++) {
      ...
      if (0 != $(selector).length) {
        ...
        DC_draggable[element]['css_initial'] = {
        ...
        if (undefined != DC_draggable[element]['checkbox']) {
          $(selector + ' div')
            .first()
            .before(DC_draggable[element]['checkbox']);

          // enable or disable element's drag at checkbox click
          $('input[type=checkbox][name=' + element + '].DC_draggable').click(
            function () {
              if ($(this).is(':checked')) enableDrag($(this).attr('name'));
              else disableDrag($(this).attr('name'));
          ...
          $(selector).mouseup(function () {
            if (dragging) {
              ...
              $('#' + $(this).attr('id')).css(
                'backgroundColor',
                DC_draggable[$(this).attr('id')]['backgroundColor'],
              );

              if ('initial' != preset) {
                DC_draggable[$(this).attr('id')]['offset_' + preset] = $(
                  '#' + $(this).attr('id'),
                ).offset();
              ...
              if (event_debug)
                if (!dragging)
                  console.log('end of drag: ' + $(this).attr('id'));
            }
          });
  ...
  function reinitalize(id, offset, forced) {
    if (
      (true == forced || true == DC_draggable[id]['draggable']) &&
      0 != $('#' + id).length
    ) {
      ...
      if (
        undefined == DC_draggable[id]['offset_' + offset] ||
        0 == offset ||
        (DC_draggable[id]['offset_' + offset].top ==
          DC_draggable[id]['offset_initial'].top &&
          DC_draggable[id]['offset_' + offset].left ==
            DC_draggable[id]['offset_initial'].left)
      ) {
  ...
  $(document).ajaxComplete(function (a, b, c) {
    if (c.url == '/Interface/Fight') {
      ...
      DC_draggable_fight['zone_informations_combat'] =
        DC_draggable['zone_informations_combat'];
  ...
  $('#bandeau .menus > li')
    .first()
    .before(
      '<li id="DC_draggable" class="couleur5"><img src="http://image.noelshack.com/fichiers/2014/23/1401993008-lado-head.png" class="infoAide" title="Une Ladoria" alt="Une Ladoria"><ul><li class="separator">Couleur de fond lors du drag:<input type="text" name="backgroundColor" class="textbox backgroundColor" value="' +
        background_draggable +
        '"></li><li name="enable" class="button">Tout activer</li><li>Configurations<div class="presetContainer">' +
        build_preset_html() +
        '</div></li><li name="bug_report" class="button separator">Signaler un bug</li><li name="suggest" class="button">Suggérer un truc</li><li class="button"><a target="_blank" href="https://greasyfork.org/scripts/2035-draggable-dc"><div>Mettre à jour</div></a></li></ul></li><li class="separator"></li>',
    );

  // global style
  $('head').append(
    '<style>#DC_draggable {padding-top: 0px !important;padding-bottom: 0px !important;}#DC_draggable ul {margin-top: 28px !important;width: 168px !important;}#DC_draggable li {padding-right: 7px !important;}#DC_draggable:hover > ul {display: block !important;}#DC_draggable .button {cursor: pointer;}#DC_draggable .textbox {width: 150px;background-color: #ffffff;}#DC_draggable .textbox.backgroundColor {margin-top: 3px;}#DC_draggable .center {text-align: center;width: 168px;color: white;font-size: 1.4em;}#DC_draggable .presetContainer {overflow: hidden;}#DC_draggable .preset {float: left;overflow: hidden;color: #000000;border: 1px solid black;padding: 1px 1px 1px 1px;width: 73px;}#DC_draggable .preset .btnTxt {margin: 1px 1px 1px 1px;padding: 1px 3px;cursor: pointer;}#DC_draggable .preset .btnTxt {width: 26px;}#DC_draggable .preset .load {float: left;}#DC_draggable .preset .save {float: right;}#DC_draggable .preset .base {width: 64px;height: 38px;}#DC_draggable .preset .base div {top: 50%;margin-top: -0.7em;}#DC_draggable .preset.selected {border-color: green;}#DC_draggable .preset .textbox {width: 69px;margin-bottom: 1px;}</style>/* to minimize [\n\t\r] */',
  );
  ...
  $('#DC_draggable [name=bug_report]').click(function () {
    ...
    $('#db_new_message:last .head .title').html(
      'Reporter un BUG sur DC Draggable',
    );
  ...
  $('#DC_draggable [name=suggest]').click(function () {
    ...
    $('#db_new_message:last').zIndex('1000');
    $('#db_new_message:last .head .title').html(
      'Suggérer un truc pour DC Draggable',
    );
    $('#db_new_message:last #nm_cible input').val('Ladoria');
    $('#db_new_message:last #nm_sujet input').val(
      '[HRP][DC Draggable][Suggestion]',
    );
    $('#db_new_message:last #nm_texte textarea').focus();
  });
  ...
  $('#DC_draggable [name=enable]').click(function () {
    ...
    $('input[type=checkbox].DC_draggable').each(function () {
      ...
      if (enable) enableDrag($(this).attr('name'));
      else disableDrag($(this).attr('name'));
    });
  });
  ...
  $('#DC_draggable .preset .btnTxt').click(function () {
    if (global_debug)
      console.log('setting for preset: ' + $(this).attr('name'));
    ...
    if ($(this).hasClass('load')) {
      ...
      for (var i = 0; i < to_save.length; i++) {
        DC_draggable[to_save[i]]['offset_' + $(this).attr('name')] = $(
          '#' + to_save[i],
        ).offset();
      ...
      saveCookie(
        'preset_aliases_' + $(this).attr('name'),
        $('#preset_aliases_' + $(this).attr('name')).val(),
      );
    }
  });
  ...
  $('.preset_aliases').keyup(function (e) {
    if (e.keyCode == 13) {
      ...
      saveCookie(
        'preset_aliases_' + $(this).attr('name'),
        preset_aliases[parseInt($(this).attr('name'))],
      );
    }
  });
});
...
function saveCookie(name, val) {
  ...
  document.cookie =
    name + '=' + val + ';expires=Wed, 01 Jan 2020 00:00:00 GMT; path=/';
}
...
function getCookie(var_name) {
  ...
  if (global_debug)
    console.log(var_name + ': ' + document.cookie.substring(start, end));
    ...
    switch (variables[i]) {
      case 'offset':
        for (var j = 1; j < preset_count + 1; j++) {
          if (
            undefined != getCookie(variable_name + '_top_' + j) &&
            undefined != getCookie(variable_name + '_left_' + j)
          ) {
            if (global_debug)
              console.log('Loading: ' + variable_name + '_' + j);
      ...
      case 'offset':
        for (var j = 1; j < preset_count + 1; j++) {
          ...
          if (undefined != DC_draggable[element]['offset_' + j]) {
            saveCookie(
              variable_name + '_top_' + j,
              DC_draggable[element]['offset_' + j].top,
            );
            saveCookie(
              variable_name + '_left_' + j,
              DC_draggable[element]['offset_' + j].left,
            );
          }
        }
        break;
...
function build_preset_html() {
  ...
  for (var i = 0; i < preset_count + 1; i++) {
    ...
    if (0 == i) {
      preset_html +=
        '<div class="DC_draggable btnTxt load base" name="' +
        i +
        '"><div>Initiale</div></div>';
    } else {
      preset_html +=
        '<div class="DC_draggable"><input type="text" id="preset_aliases_' +
        i +
        '" class="DC_draggable textbox preset_aliases" name="' +
        i +
        '" value="' +
        (undefined != preset_aliases[i - 1]
          ? preset_aliases[i - 1]
          : 'Conf. ' + i) +
        '" onclick="this.select();"></div>';

      preset_html +=
        '<div class="DC_draggable btnTxt save" name="' + i + '">Save</div>';
      preset_html +=
        '<div class="DC_draggable btnTxt load" name="' + i + '">Load</div>';
    }
    preset_html += '</div>';
  }
