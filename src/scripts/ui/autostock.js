...
$(document).ajaxSuccess(function (e, xhr, opt) {
  if (
    document.getElementById('modif_stocks_form') != null &&
    Global_GUIActive == false
  ) {
    htmlstockclass = document
      .getElementById('modif_stocks_form')
      .getElementsByClassName('stock');
...
function AfficherAutoStockBoutique() {
  var menu = document.createElement('div');
  menu.innerHTML =
    '<span style="left:135px;">AutoStock : Boutique à venir.</span>';
  menu.setAttribute('id', 'AutoStockMenu');
...
function AfficherAutoStockUsine() {
  var menu = document.createElement('div');
  menu.innerHTML =
    '<span id="AutoStockRootMenu" style="left:25px;"><a id="MettreEnVenteUsine" style="cursor:pointer;">Mettre en vente</a> | <a id="ChangePrixUsine" style="cursor:pointer;">Changer prix</a> | <a id="ExportPrixUsine" style="cursor:pointer;">Exporter prix</a> | <a id="ImportPrixUsine" style="cursor:pointer;">Importer prix</a></span>' +
    '<div id="AutoStockListAnchor" style="left:515px; top:-498px; width:1px; height:1px; background-color:red">' +
    '<div id="AutoStockLog" style="height:518px; width:500px; background-color:#0B4E64; overflow: auto;">Autostock v' +
    versionAutoStock +
    ' par Amane-Mochizuki<br />journal de traitement :</div></div>';
  menu.setAttribute('id', 'AutoStockMenu');

  document.getElementById('modif_stocks_form').appendChild(menu);

  document
    .getElementById('MettreEnVenteUsine')
    .addEventListener('click', MettreEnVenteUsine, false);
  document
    .getElementById('ChangePrixUsine')
    .addEventListener('click', ChangePrixUsine, false);
  document
    .getElementById('ExportPrixUsine')
    .addEventListener('click', ExportPrixUsine, false);
  document
    .getElementById('ImportPrixUsine')
    .addEventListener('click', ImportPrixUsine, false);
}
...
function MettreEnVenteUsine(zEvent) {
  OutputLog(
    '<span style="color:black;">------------------ Traitement de mise en vente ------------------</span>',
  );
  var htmlNode;
  ...
  var ReserveStock = prompt(
    'Autostock : \n Seuil de réserve exclue de la vente pour produits standard \n champs vide = 0',
  );
  if (ReserveStock == null) {
    OutputLog(
      '<span style="color:black;">Paramètre de réserve standard : </span> Opération annulée',
    );
    OutputLog(
      '<span style="color:black;">---------------- Fin Traitement de mise en vente ----------------</span>',
    );
    return 0;
  }
  ...
  if (isdigit(ReserveStock) || ReserveStock == 0) {
    OutputLog(
      '<span style="color:black;">Paramètre de réserve standard : </span>' +
        ReserveStock,
    );
  } else {
    OutputLog(
      '<span style="color:black;">Paramètre de réserve standard invalide : </span>' +
        ReserveStock,
    );
    OutputLog(
      '<span style="color:black;">---------------- Fin Traitement de mise en vente ----------------</span>',
  ...
  var ReserveStockAliments = prompt(
    'Autostock : \n Seuil de réserve exclue de la vente pour produits alimentaires \n champs vide = 0',
  );
  if (ReserveStockAliments == null) {
    OutputLog(
      '<span style="color:black;">Paramètre de réserve aliments : </span> Opération annulée',
    );
    OutputLog(
      '<span style="color:black;">---------------- Fin Traitement de mise en vente ----------------</span>',
    );
  ...
  if (isdigit(ReserveStockAliments) || ReserveStockAliments == 0) {
    OutputLog(
      '<span style="color:black;">Paramètre de réserve aliments : </span>' +
        ReserveStockAliments,
    );
  } else {
    OutputLog(
      '<span style="color:black;">Paramètre de réserve aliments invalide : </span>' +
        ReserveStockAliments,
    );
    OutputLog(
      '<span style="color:black;">---------------- Fin Traitement de mise en vente ----------------</span>',
    ...
    if (UnObjet_Type == 'Stock de 10 aliments') {
      ...
      OutputLog(
        '<span style="color:#fff;">' +
          UnObjet_Nom +
          '</span> en vente : <span class="couleur5">' +
          UnObjet_StockVente +
          '</span> > <span class="couleur1">' +
          MaxStockVente +
          '</span> sur total <span class="couleur0">' +
          UnObjet_StockDispo +
          '</span> (réserve ' +
          ReserveStockAliments +
          ')',
...
function ChangePrixUsine(zEvent) {
  OutputLog(
    '<span style="color:black;">---------------- Traitement de changement prix ----------------</span>',
  );
  var htmlNode;
  var elements = document
    .getElementById('modif_stocks_form')
  ...
  var MargeAppliquer = prompt(
    'Autostock : \n Marge a appliquer sur le prix de production (en %) \n champs vide = 0%',
  );
  if (MargeAppliquer == null) {
    OutputLog(
      '<span style="color:black;">Paramètre de marge a appliquer: </span> Opération annulée',
    );
    OutputLog(
      '<span style="color:black;">---------------- Fin Traitement de changement prix ----------------</span>',
    );
    return 0;
  }
  ...
  OutputLog(
    '<span style="color:black;">Paramètre de marge a appliquer : </span>' +
      MargeAppliquer +
      '%',
  );
  ...
  for (var i = 0; i < elements.length; i++) {
    ...
    if (UnObjet_PrixProd != 0) {
      ...
      OutputLog(
        '<span style="color:#fff;">' +
          UnObjet_Nom +
          '</span> prix de vente : <span class="couleur5">' +
          UnObjet_PrixVente +
          '</span> > <span class="couleur1">' +
          NouveauPrixVente +
          '</span> (marge ' +
      ...
      OutputLog(
        '<span style="color:#ff0;">/!\\ Modification ignoré car le prix de prod. est de 0¢r (Ancienne Génération ?)',
      );
      OutputLog(
        '<span style="color:#ff0;">--> </span> ' + UnObjet_Nom + '</span>',
      );
    }
  }
  OutputLog(
...
function ExportPrixUsine(zEvent) {
  OutputLog(
    '<span style="color:black;">--------------- Traitement pour export des prix --------------</span>',
  );
  var uneligne = '';
  ...
  htmlNode.innerHTML =
    '<textarea disabled name="PrixExport" cols="55" rows="16">' +
    csv +
    '</textarea>';
  document.getElementById('AutoStockLog').appendChild(htmlNode);

  OutputLog(
    '<span style="color:black;">------------- Fin Traitement pour export des prix -------------</span>',
  );
}

function ImportPrixUsine(zEvent) {
  OutputLog(
    '<span style="color:black;">--------------- Traitement pour import des prix --------------</span>',
  );
  var uneligne = '';
  var csv = 'ProductID;Designation;Prix\n';
  ...
  htmlNode = document.createElement('p');
  htmlNode.innerHTML =
    '<span class="ZonePrixImport"><textarea id="PrixImport" cols="55" rows="6"></textarea></span><br /><span class="ConfirmerImport"><a id="AnnulerImportUsine" style="cursor:pointer;">Annuler</a> | <a id="ValiderImportUsine" style="cursor:pointer;">Valider</a></span>';
  document.getElementById('AutoStockLog').appendChild(htmlNode);

  $('#AutoStockRootMenu').children().prop('hidden', true);
  document
    .getElementById('AnnulerImportUsine')
    .addEventListener('click', AnnulerImportUsine, false);
  document
    .getElementById('ValiderImportUsine')
    .addEventListener('click', ValiderImportUsine, false);
}
...
function AnnulerImportUsine(zEvent) {
  ...
  OutputLog(
    '<span style="color:black;">------------ Fin traitement pour import des prix -------------</span>',
  );
}
...
function ValiderImportUsine(zEvent) {
  ...
  for (var i = 0; i < LignesImporte.length; i++) {
    ...
    if (
      DonneeUneLigne[0] != undefined &&
      DonneeUneLigne[1] != undefined &&
      DonneeUneLigne[2] != undefined
    ) {
      ...
      if (
        UnProductID == 'ProductID' &&
        UneDesignation == 'Designation' &&
        UnPrix == 'Prix' &&
        i == 0
      ) {
        OutputLog(
          "démarrage de l'importation : la structure semble cohérente.",
        );
      } else {
        if (i != 0) {
          idTrouve = false;
              ...
              if (UnObjet_Nom == UneDesignation) {
                ...
                OutputLog(
                  '<span style="color:#f00;">' +
                    ...
                    " Erreur l'ID produit " +
                    UnObjet_ID +
                    ' a été trouvé avec un autre nom (ligne ignorée) :</span>',
                );
                OutputLog(
                  '<span style="color:#f00;">--> </span> importé : <span style="color:#fff;">' +
                    UneDesignation +
                    '</span> / stock : <span style="color:#fff;">' +
                    UnObjet_Nom +
                    '</span>',
                );
              }
          ...
          if (idTrouve == false) {
            ...
            OutputLog(
              '<span style="color:#ff0;">' +
                ...
                " L'ID produit " +
                UnProductID +
                " n'a pas été trouvé dans le stock et à été ignoré",
            );
            OutputLog(
              '<span style="color:#ff0;">--> </span> Stock manquant : <span style="color:#fff;">' +
                UneDesignation +
                '</span>',
            );
          }
          ...
          OutputLog(
            '<span style="color:#f00;">' +
              GUIerror +
              " Erreur de structure sur données, l'entête de contrôle est absent ou invalide.</span>",
          );
      ...
      if (UneLigneImporte == '') {
        ...
        OutputLog(
          '<span style="color:#f00;">' +
            GUIerror +
            " Erreur de structure sur ligne : l'entrée suivante a été ignoré.",
        );
...
function DetectionUiStock(stringhtmlStock) {
  ...
  if (
    stringhtmlStock.includes('Stock de') &&
    stringhtmlStock.includes('<input id="champ_cpt_') &&
    stringhtmlStock.includes('<input id="champ_stock_')
  ) {
    resultat = 'Usine';
  } else {
    if (
      stringhtmlStock.includes('<td id="prix_unitaire_') &&
      stringhtmlStock.includes('<input id="champ_stock_')
    ) {
      resultat = 'Boutique';
    }
...
function OutputLog(HTMLstring) {
  ...
  htmlNode.setAttribute(
    'style',
    'font-family:"Trebuchet MS",Verdana,Arial,sans-serif; color: #00e1ed; font-variant: small-caps; font-size: 13px;',
  );
  document.getElementById('AutoStockLog').appendChild(htmlNode);
}
...
function ScanHtmlUnObjetUsine(html) {
  ...
  unitem_nom = Extractsubstring(
    unitem_temp,
    '<span class="couleur4">',
    '</span>',
  );

  unitem_stockdispo = Extractsubstring(
    unitem,
    '<td class="quantite_vente type2" style="height:20px;width:50px;">',
    '</td>',
  );
  unitem_prixprod = Extractsubstring(
    unitem,
    '<td id="prix_unitaire_' +
      unitem_id +
      '" class="prix type2" style="padding-left:20px;width:75px;">',
    '</td>',
  );
  unitem_stockvente = Extractsubstring(
    unitem,
    'name="cpt_' + unitem_id + '" value="',
    '">',
  );
  unitem_prixvente = Extractsubstring(
    unitem,
    'name="stock_' + unitem_id + '" value="',
    '">',
  );
...
function CleanHtml(SringANettoyer) {
  var newPageTxt = SringANettoyer.split('\n')
    .map((line) => line.trim())
    .join(' ');
