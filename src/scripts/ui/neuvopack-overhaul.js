...
const getProgressBarElement = (id, fillLevel) => {
    ...
    <div>
        <div>
            ...
            <svg width="150px" viewBox="0 0 110 6">
                ...
                <line id="neuvopack_${id}_fill_bar" class="no_bar no_inner_bar" x1="5" x2="${
      5 + progress
    }" y1="3" y2="3"></line>
            </svg>
            <div></div>
  ...
  return `
    <div class="no_progress">
        ...
        <div id="neuvopack_${id}_fill_bar" class="no_bar" style="width: ${
    fillLevel / 20.2
  }%;background: linear-gradient(${fillColor});"></div>
    </div>`;
};
...
const getNeuvoInfo = (id, fillLevel, durability, maxDurability) => `
      ...
      <div class="no_neuvo_info_item">
        ...
        ${getNeuvoDurability(
          id,
          durability,
        )}<span id="no_neuvopack_${id}_durability_max" class="couleur5">${
  maxDurability.startsWith('/') ? maxDurability : '/' + maxDurability
}</span>
      </div>
    `;
...
const recalculateAllStatuses = () => {
  ...
  $(`#no_neuvopack_${activeNeuvopackId}_timer_text`)
    .empty()
    .append(getDefaultFormattedTime());
  $(`#no_neuvopack_${activeNeuvopackId}_timer_inner_circle`).attr(
    'stroke-dasharray',
    `0 100`,
  );
  ...
  neuvopacks.map((k, v) => {
    ...
    if (isActive) {
      ...
      $(getNeuvoInfo(id, fillLevel, durability, maxDurability)).appendTo(
        $(`#no_neuvopack_${id}_info`),
      );
    }
  });
  ...
  if (initialTime !== 0) {
    ...
    $(`#no_neuvopack_${activeNeuvopackId}_timer_inner_circle`).attr(
      'stroke-dasharray',
      `${100 - remainingProgress} ${remainingProgress}`,
    );
  }
};
...
const updateNeuvopack = (xmlInfo) => {
  const neuvopacks = $(xmlInfo)
    .find('.objet_type_Neuvopack.activable')
    .parent();
...
const setFillLevel = (id, fillLevel) => {
  ...
  if (progressBarStyle === 'monochrome') {
    ...
    $(`#neuvopack_${id}_fill_bar`).css(
      'background',
      `linear-gradient(${fillColor})`,
    );
  }
};
...
const updateResearchTimer = () => {
  ...
  if (
    action === 'Vous êtes en train de fouiller la zone' &&
    precision !== 'Cristaux trouvés !'
  ) {
    const components = precision
      .replace('Prochain résultat dans ', '')
      .replace('min', '')
      .replace('sec', '')
      .split(' ');
...
const getDefaultFormattedTime = () =>
  '<tt>--<span class="couleur5">m</span><br/>--<span class="couleur5">s</span></tt>';
...
const createNeuvopackElement = (neuvopack) => {
  ...
  if (!isAlreadyCreated) {
    const fillLevel = $(neuvopack)
      ...
      .children('[class^=contenance_appareil]')
      .text();
    const durability = $(neuvopack)
      .children('.infoBox')
      .children('.infoBox_content')
      .children('.info_objet')
      .children('.durabiliteinfo ')
      .children('.item_pv')
      .text();
    const maxDurability = $(neuvopack)
      .children('.infoBox')
      .children('.infoBox_content')
      .children('.info_objet')
      .children('.durabiliteinfo ')
      .children('.item_pv_max')
      .text();

    getNeuvopackElement(id, fillLevel, durability, maxDurability).appendTo(
...
$(document).ready(() => {
  setTimeout(() => {
    $(
      '<div id="zone_neuvopack_displayed" class="zone_conteneurs_displayed "></div>',
    ).appendTo('body');
    findAndCreateNeuvopacks();

    intercept('https://www.dreadcast.net/Check', (e) =>
      updateNeuvopack(e.response),
    );
    intercept('https://www.dreadcast.net/Main/InventoryBox', (e) =>
      updateNeuvopack(e.response),
    );
    intercept(
      'https://www.dreadcast.net/Item/Move',
      recalculateStatusesOnItemMove,
    );

    $('#action_actuelle').on('DOMSubtreeModified', updateResearchTimer);

    $('#annexe_inventaire_ext').on('DOMSubtreeModified', () => {
      const _neuvopacks = $('.objet_type_Neuvopack.ui-draggable').parent();
      _neuvopacks.unbind('click');
      setActionsOnNeuvopacks(_neuvopacks);
    });
    $('#zone_conteneurs_displayed').on('DOMSubtreeModified', () => {
      const _neuvopacks = $('.objet_type_Neuvopack.ui-draggable').parent();
      _neuvopacks.unbind('click');
      setActionsOnNeuvopacks(_neuvopacks);
      findAndCreateNeuvopacks();
