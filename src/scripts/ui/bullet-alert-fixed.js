...
$('body').append(
  '<audio id="DC_bullets_low_charger" src="https://bacon-network.net/dreadcast/bip.mp3" type="audio/waw"></audio><audio id="DC_bullets_empty_charger" src="https://bacon-network.net/dreadcast/low_bullet.mp3" type="audio/waw"></audio>',
);
...
$(document).ready(function () {
  function alert_user() {
    ...
    if (0 == gun_chargers.length) return;
    ...
    for (var key in bullet_chargers) {
      //console.log(key);
      if ('empty' != alert) {
        if ('empty' == bullet_chargers[key].alert) alert = 'empty';
        if ('low' == bullet_chargers[key].alert) alert = 'low';
      }
    ...
    var song =
      'empty' == alert
        ? (song = $('#DC_bullets_empty_charger')[0])
  ...
  function animate_gun(case_number, color) {
    ...
    if ('red' == color) rgba = 'rgba(255,0,0,0.5)';
    if ('yellow' == color) rgba = 'rgba(255,125,0,0.5)';

    $('#equipement_inventaire .zone_case' + (case_number + 3))
      .find('img.activable')
