...
var datePickerOutput =
  (('' + dayPicker).length < 2 ? '0' : '') +
  dayPicker +
  '/' +
  (('' + monthPicker).length < 2 ? '0' : '') +
  monthPicker +
  '/' +
  ...
  $(document).ajaxSuccess(function (e, xhr, opt) {
    if (opt.url.includes('/CrystalHistory')) {
      if ($('#datepicker').length == 0) {
        $('#db_historique_cristaux').append(
          '<form id="historyCalendarForm" class="longTextInput"/>',
        );
        $('#historyCalendarForm').append(
          '<div id="historyCalendarInput" class="mediumTextInput"></div>',
        );
        $('#historyCalendarInput').append('<div class="left"></div>');
        $('#historyCalendarInput').append('<div class="right"></div>');
        $('#historyCalendarInput').append(
          '<input type="text" id="datepicker" class="variableTextInput" />',
        );
        ...
        function generateDateTimestamp() {
          ...
          var formattedDateInShittyUSFormat =
            formatDateToShittyUSFormat[1] +
            '/' +
            formatDateToShittyUSFormat[0] +
            '/' +
            formatDateToShittyUSFormat[2];
          generatedTimestamp =
            new Date(formattedDateInShittyUSFormat).getTime() / 1000;
        ...
        function generateHistoryURL() {
          ...
          var historyURL =
            'Main/DataBox/default=CrystalHistory&date=' + generatedDate;
          return historyURL;
        }
        generateCalendar();

        var $calendarButton = $('#historyCalendarInput').append(
          '<div class="mediumValidInput" id="calendarButton"></div>',
        );
        $calendarButton.click(function () {
          engine.regenerateDataBox(generateHistoryURL());
        });
      }

      var $button = $('#db_historique_cristaux').append(
        '<div id="downloadCurrentHistoryContainer">',
      );
      $('#downloadCurrentHistoryContainer').append(
        '<button id="downloadCurrentHistory" class="btnTxt">Copier</button>',
      );
      $button.click(function () {
        var copyText = $('#entreprise_historique')
          .children()
          .first()
          .text()
          .replace(/(\r\n|\n|\r|\t|")/gm, '')
    ...
    if (opt.url.includes('/ListeCryoFull')) {
      var $button = $('#db_liste_cryo').append(
        '<button id="downloadCurrentHistory" class="btnTxt">Copier</button>',
      );
      $button.click(function () {
        var copyText = $('#imperial_data')
          .text()
          .replace(/(\r\n|\n|\r|\t|")/gm, '')
          .replace(/\s+/g, ' ')
          .replace(/\[/g, '\r\n[');
    ...
    if (opt.url.includes('/DataBox/default=CompanyHistory')) {
      if ($('#datepicker').length == 0) {
        $('#db_historique_entreprise').append(
          '<form id="historyCalendarForm" class="longTextInput"/>',
        );
        $('#historyCalendarForm').append(
          '<div id="historyCalendarInput" class="mediumTextInput"></div>',
        ...
        function generateHistoryURL() {
          ...
          if ($('#companyId').length == 0) {
            $('#db_historique_entreprise').append(
              '<div id=companyId>' + companyId + '</div>',
            );
          } else {
          ...
          var generatedDate = generateDateTimestamp();
          var historyURL =
            'Main/DataBox/default=CompanyHistory&company=' +
            companyId +
            '&date=' +
            generatedDate;
          return historyURL;
      ...
      if ($('#downloadCurrentHistory').length == 0) {
        ...
        var $button = $('#downloadCurrentHistoryContainer').append(
          '<button id="downloadCurrentHistory" class="btnTxt">Copier</button>',
        );
